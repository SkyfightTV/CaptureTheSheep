...
import fr.skyfighttv.cts.commands.subcommands.CTSSetZone;
...
public class SheepManager {
    ...
    public static void setPassenger(Player player, Sheep sheep) {
        ...
        if (sheepGames.containsKey(world)
            ...
            if (sheepGames.get(world).get(CTSSetSheep.getBlue()).equals(sheep)) {
                sheepTeam = CTSSetSheep.getBlue();
                zoneObjective = CTSSetZone.getRed();
            } else if (sheepGames.get(world).get(CTSSetSheep.getRed()).equals(sheep)) {
                sheepTeam = CTSSetSheep.getRed();
                zoneObjective = CTSSetZone.getBlue();
            } else return;

            if ((sheepTeam.equals(CTSSetSheep.getBlue())
                    && GameManager.getBlueTeam().get(world).contains(player))
                    || (sheepTeam.equals(CTSSetSheep.getRed())
